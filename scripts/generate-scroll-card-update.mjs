#!/usr/bin/env node
/**
 * Web Reference Lab — Category Generator: Scroll-Card-Update (v4)
 * base44.com Playwright 실측 기반 100% 시각 매칭
 *
 * v4 핵심: base44.com 정확 토큰 — border-radius 9px, grid 59:41,
 *   카드별 고유 그라디언트, 본문 30px/400, 번호 19px/400,
 *   패턴 01은 개별 sticky 카드 (base44 시그니처), 02~10은 sticky-stage 모델
 *
...
const CATEGORY = {
  ...
  summary: 'base44.com 시그니처 — 스크롤 시 카드(좌 텍스트 + 우 이미지)가 아래에서 위로 올라와 이전 카드를 덮는 인터랙션. 개별 sticky 카드 모델(패턴 01)과 sticky-stage 모델(패턴 02~10)로 10가지 전환 방식 비교. Playwright 실측 토큰(border-radius 9px, grid 59:41, 카드별 그라디언트, 본문 30px weight 400) 100% 반영.'
};
...
function stickyCardsMarkup() {
  return SCU_CARDS.map(function (c, i) {
    var card = '<div class="scu-card" data-i="' + i + '" style="background:' + GRADIENTS[i] + '">'
      + '\n          ' + cardInnerHTML(c)
      + '\n        </div>';
    return i < N - 1 ? card + '\n        <div class="scu-spacer"></div>' : card;
  }).join('\n        ');
}

function absoluteCardsMarkup() {
  return SCU_CARDS.map(function (c, i) {
    return '<div class="scu-card' + (i === 0 ? ' is-on' : '') + '" data-i="' + i + '" style="background:' + GRADIENTS[i] + '">'
      + '\n          ' + cardInnerHTML(c)
      + '\n        </div>';
  }).join('\n        ');
}
...
/* ================================================================
   CSS — base44 실측 토큰 (Playwright 정량 데이터)
   ================================================================
...
const STICKY_CSS = ''
  ...
  + '.scu-card { position: sticky; top: 0; height: 100vh; display: flex; align-items: center; justify-content: center; }\n'
  + '.scu-spacer { height: 10vh; }\n'
  + INNER_CSS;

const STAGE_CSS = ''
  + '.scroll-track { min-height: 500vh; position: relative; }\n'
  + '.sticky-stage { position: sticky; top: 0; height: 100vh; overflow: hidden; background: ' + GRADIENTS[0] + '; }\n'
  + '.scu-card { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; will-change: transform, opacity, clip-path, filter; }\n'
  + INNER_CSS;
...
const CALC_FN = 'var track = document.querySelector(".scroll-track");\n'
  + 'function calc(){\n'
...
const SNIPPET_HTML_STICKY = '<div class="scroll-track">\n  <div class="card" data-i="0" style="background:linear-gradient(...)">\n    <div class="inner">\n      <div class="text">넘버링 + 타이틀 + 본문 + CTA</div>\n      <div class="img-wrap"><img src="..." alt=""></div>\n    </div>\n  </div>\n  <div class="spacer"></div>\n  <!-- x4 개별 sticky 카드 + 사이 spacer -->\n</div>';

const SNIPPET_HTML_STAGE = '<div class="scroll-track">\n  <div class="sticky-stage">\n    <div class="card" data-i="0" style="background:linear-gradient(...)">\n      <div class="inner">\n        <div class="text">넘버링 + 타이틀 + 본문 + CTA</div>\n        <div class="img-wrap"><img src="..." alt=""></div>\n      </div>\n    </div>\n    <!-- x4 카드 (각자 그라디언트 배경) -->\n  </div>\n</div>';
...
const PATTERNS = [
  // 01 — stack-up (base44 시그니처 — 개별 sticky 카드)
  {
    id: 'stack-up', num: '01', title: '스택 업 (base44 시그니처)',
    summary: '카드(텍스트+이미지)가 CSS position:sticky로 개별 고정되어, 스크롤 시 다음 카드가 자연스럽게 위에 덮임. base44.com 시그니처 인터랙션을 Playwright 실측 토큰으로 100% 재현.',
    demo: {
      sticky: true,
      css: STICKY_CSS,
    ...
    snippetHTML: SNIPPET_HTML_STICKY,
    snippetCSS: '.scroll-track { position: relative; }\n.card { position: sticky; top: 0; height: 100vh;\n  display: flex; align-items: center; justify-content: center;\n  background: linear-gradient(...); /* 카드별 고유 그라디언트 */ }\n.spacer { height: 10vh; }\n.inner { width: 65vw; max-width: 1100px; display: grid;\n  grid-template-columns: 59fr 41fr; background: #fff;\n  border-radius: 9px; overflow: hidden; aspect-ratio: 1100/587; }',
    snippetJS: '// CSS position:sticky가 스택 업을 처리\n// JS는 progress bar만 업데이트\nvar p = calc();\nprogressFill.style.width = (p * 100) + "%";',
    explain: 'base44.com의 시그니처 인터랙션을 CSS position:sticky로 구현. 각 카드는 개별적으로 sticky top:0이 적용되어, 스크롤 시 DOM 순서에 따라 다음 카드가 이전 카드 위에 자연스럽게 덮인다. JS는 transform이 아닌 progress bar 업데이트만 담당. spacer(10vh)가 카드 간 전환 타이밍을 제어.',
    kv: [
      { label: '의존성', value: 'CSS only (JS는 progress bar만)' },
      { label: '트리거', value: 'CSS position:sticky (네이티브 스택)' },
      { label: '카드 구조', value: '좌 텍스트(59%) + 우 이미지(41%) grid' },
      { label: '이너 카드', value: '1100px, border-radius 9px, #fff, 그림자 없음' },
      { label: '그라디언트', value: '카드별 고유 4종 (base44 실측)' },
      { label: '시그니처', value: 'base44.com Playwright 100% 매칭' }
    ],
    guide: 'CSS sticky만으로 구현 가능한 가장 자연스러운 카드 스택. JS transform이 없어 jank 없음. spacer 높이로 전환 속도 제어 (10vh ≈ 카드 높이의 10%). 모바일에서는 grid를 1-column으로 fallback 권장.',
    recommendations: [
      { place: '히어로 헤더', body: '제품 핵심 가치 4가지 — base44 시그니처 그대로 적용' },
      { place: '랜딩 페이지', body: '기능 소개 섹션 — 자연스러운 카드 스택으로 순차 전달' },
      { place: '제품 섹션', body: '제품 라인업 비교 — 카드별 이미지+스펙 교체' },
      { place: '포트폴리오 소개', body: '대표 작품 케이스 — 좌측 설명 + 우측 작품 이미지' }
    ],
    tradeoff: 'CSS sticky 전용이라 전환 효과 커스터마이징 불가. 카드 4~6개 적정. 모바일 Safari에서 sticky 렌더링 테스트 필요.'
  },

  // 02 — fade-stack
  {
    id: 'fade-stack', num: '02', title: '페이드 스택',
    summary: '활성 카드만 opacity 1, 나머지는 0. CSS transition 600ms로 부드러운 교체.',
    demo: {
      css: STAGE_CSS + '.scu-card { opacity: 0; transition: opacity 600ms ease-in-out; }\n.scu-card.is-on { opacity: 1; }',
      script: STAGE_PRE + '\nfunction applyReveal(p){\n  var idx = Math.min(Math.floor(p * N), N - 1);\n  cards.forEach(function(c, i){ c.classList.toggle("is-on", i === idx); });\n}',
      height: 480
    },
    snippetHTML: SNIPPET_HTML_STAGE,
    snippetCSS: '.card { position: absolute; inset: 0;\n  opacity: 0; transition: opacity 600ms ease-in-out; }\n.card.is-on { opacity: 1; }\n.inner { width: 65vw; max-width: 1100px; display: grid;\n  grid-template-columns: 59fr 41fr; background: #fff;\n  border-radius: 9px; overflow: hidden; }',
    snippetJS: 'var idx = Math.min(Math.floor(p * N), N - 1);\ncards.forEach(function(c, i){\n  c.classList.toggle("is-on", i === idx);\n});',
    explain: '활성 카드(텍스트+이미지 전체)만 .is-on 클래스로 opacity 1. CSS transition 600ms ease-in-out으로 부드러운 교차. 모든 카드가 absolute로 쌓여 있고 활성 카드만 보임. 그라디언트 배경도 함께 fade.',
    kv: [
      { label: '의존성', value: 'CSS only (JS는 클래스 토글)' },
      { label: '트리거', value: '스크롤 진행률 → activeIndex' },
      { label: '매핑', value: 'idx = floor(p × N)' },
      { label: 'Fade', value: '600ms ease-in-out' },
      { label: '카드 구조', value: '좌 텍스트(59%) + 우 이미지(41%)' },
      { label: '시그니처', value: 'Stripe / Vercel 부드러운 swap' }
    ],
    guide: '가장 안전하고 보편적인 카드 전환. 카드 간 톤이 비슷할 때 자연스러움. transition 500~800ms 권장. 그라디언트 배경도 카드와 함께 fade 되어 전환 간 색상 블렌딩.',
    recommendations: [
      { place: '히어로 헤더', body: 'SaaS 메인 hero — 안전하고 부드러운 전환' },
      { place: '랜딩 페이지', body: '기능 소개 — 산만하지 않은 교체' },
      { place: '제품 섹션', body: '제품 변형 비교 — 배경 그라디언트 변화 포함' },
  ...
  // 03 — scale-pop
  {
    ...
    summary: '활성 카드 scale 1, 다음 카드 0.92(작게 대기), 이전 카드 1.04(살짝 커지며 퇴장).',
    demo: {
      css: STAGE_CSS + '.scu-card { opacity: 0; transform: scale(0.92); transition: opacity 500ms ease-out, transform 500ms cubic-bezier(0.2,0,0,1); }\n.scu-card.is-on { opacity: 1; transform: scale(1); }',
    ...
    explain: '활성 카드 scale 1 + opacity 1. 대기 카드 scale 0.92(작게), 지난 카드 scale 1.04(살짝 커지며 사라짐). 카드 전체(텍스트+이미지+그라디언트 배경)가 함께 줌.',
    kv: [
    ...
    guide: '변화 폭이 작아(8~12%) 부드럽고 자연스러움. 카드 전체가 함께 줌 되어 텍스트+이미지+배경 동시 변화. scale 0.88~0.96 범위가 적정.',
    recommendations: [
  ...
  // 04 — slide-from-right
  {
    ...
    summary: '활성 카드 translateX(0), 이전 카드 -100%(좌측 퇴장), 다음 카드 100%(우측 대기).',
    demo: {
      css: STAGE_CSS + '.scu-card { transform: translateX(100%); opacity: 0; transition: transform 600ms cubic-bezier(0.22,1,0.36,1), opacity 400ms ease; }\n.scu-card.is-on { transform: translateX(0); opacity: 1; }',
    ...
    explain: '활성 카드 translateX(0). 지난 카드 -100%(좌측 퇴장), 다음 카드 100%(우측 대기). 카드 전체(텍스트+이미지+그라디언트 배경)가 함께 가로 이동. sticky-stage overflow:hidden으로 영역 밖 카드 숨김.',
    kv: [
      { label: '의존성', value: 'CSS + Vanilla JS' },
      { label: '트리거', value: '스크롤 진행률 → activeIndex' },
    ...
    guide: '가로 슬라이드. sticky-stage의 overflow:hidden 필수. 그라디언트 배경도 카드와 함께 슬라이드되어 색상 전환 효과.',
    recommendations: [
  ...
  // 05 — clip-bottom
  {
    ...
    summary: '카드 전체가 clip-path inset(100% 0 0 0)에서 inset(0)으로 — 아래에서 위로 커튼 reveal.',
    demo: {
      css: STAGE_CSS + '.scu-card { clip-path: inset(100% 0 0 0); }\n.scu-card[data-i="0"] { clip-path: inset(0); }',
    ...
    explain: '첫 카드는 즉시 보임. 이후 카드는 clip-path inset top 100%에서 0으로 스크롤 진행률에 따라 줄어들며 아래→위로 reveal. 카드 전체(텍스트+이미지+그라디언트)가 커튼처럼 드러남.',
    kv: [
      { label: '의존성', value: 'Vanilla JS' },
      { label: '트리거', value: '스크롤 진행률 → clip-path inset' },
      { label: '매핑', value: 'inset top = (1 - local) × 100%' },
      { label: '핵심', value: 'clip-path inset (GPU 가속)' },
      { label: '카드 구조', value: '좌 텍스트(59%) + 우 이미지(41%)' },
      { label: '시그니처', value: '시네마틱 커튼 reveal' }
    ],
    guide: '시네마틱 reveal. zIndex 누적 필수. 텍스트+이미지+그라디언트 배경이 동시에 reveal 되어 임팩트 강함. clip-path는 모던 브라우저 필수.',
    recommendations: [
  ...
  // 06 — rotate-tilt
  {
    ...
    summary: '카드 전체가 translateY ±60px + rotate ±3deg로 등장·퇴장. 종이가 떨어지는 느낌.',
    demo: {
      css: STAGE_CSS + '.scu-card { opacity: 0; transform: translateY(60px) rotate(-3deg); transition: opacity 600ms ease-out, transform 600ms cubic-bezier(0.2,0,0,1); transform-origin: bottom center; }\n.scu-card.is-on { opacity: 1; transform: translateY(0) rotate(0deg); }',
    ...
    snippetJS: 'cards.forEach(function(c, i){\n  c.classList.toggle("is-on", i === idx);\n  if (i < idx) c.style.transform = "translateY(-60px) rotate(3deg)";\n  else if (i > idx) c.style.transform = "translateY(60px) rotate(-3deg)";\n  else c.style.transform = "translateY(0) rotate(0)";\n});',
    explain: '활성 카드 translateY(0) + rotate(0). 다음 카드 60px 아래 + -3deg, 지난 카드 60px 위 + +3deg. 카드 전체(텍스트+이미지+그라디언트)가 종이처럼 기울어지며 등장·퇴장.',
    kv: [
      { label: '의존성', value: 'CSS + Vanilla JS' },
      { label: '트리거', value: '스크롤 진행률 → activeIndex' },
    ...
    guide: '회전 2~5deg가 자연스러움. transform-origin: bottom center가 핵심 — 아래를 축으로 기울어져 종이 넘김 인상. 텍스트+이미지+배경이 함께 회전.',
    recommendations: [
  ...
  // 07 — blur-swap
  {
    ...
    summary: '카드 전체가 blur(18px)↔0 + opacity 교차. 카메라 초점 변화 같은 영화적 전환.',
    demo: {
      css: STAGE_CSS + '.scu-card { opacity: 0; filter: blur(18px); transition: opacity 700ms ease-in-out, filter 700ms ease-in-out; }\n.scu-card.is-on { opacity: 1; filter: blur(0); }',
      script: STAGE_PRE + '\nfunction applyReveal(p){\n  var idx = Math.min(Math.floor(p * N), N - 1);\n  cards.forEach(function(c, i){ c.classList.toggle("is-on", i === idx); });\n}',
      height: 480
    },
    snippetHTML: SNIPPET_HTML_STAGE,
    ...
    snippetJS: 'var idx = Math.min(Math.floor(p * N), N - 1);\ncards.forEach(function(c, i){\n  c.classList.toggle("is-on", i === idx);\n});',
    explain: '비활성 카드 filter:blur(18px) + opacity 0. 활성 카드 blur(0) + opacity 1. 카드 전체(텍스트+이미지+그라디언트)가 함께 blur/unblur — 카메라 초점 전환.',
    kv: [
      { label: '의존성', value: 'CSS only (JS는 클래스 토글)' },
      { label: '트리거', value: '스크롤 진행률 → activeIndex' },
      { label: '매핑', value: 'is-on 토글 → blur(18px)↔0' },
      { label: '블러 거리', value: '18px (transition 700ms)' },
      { label: '카드 구조', value: '좌 텍스트(59%) + 우 이미지(41%)' },
      { label: '시그니처', value: 'Apple Vision Pro 시네마틱' }
    ],
    guide: '영화적 초점 전환. 텍스트까지 blur 되므로 전환 중 가독성은 떨어지지만 임팩트 강함. transition 700~1000ms. 그라디언트 배경도 blur 되어 몽환적.',
    recommendations: [
  ...
  // 08 — stack-spread
  {
    ...
    summary: '이전 카드가 -12px씩 위로 밀리고 scale 축소되어 쌓이는 카드 덱 깊이감.',
    demo: {
      css: STAGE_CSS + '.scu-card { transform: translateY(100%) scale(1); }\n.scu-card[data-i="0"] { transform: translateY(0) scale(1); }',
    ...
    snippetJS: 'cards.forEach(function(c, i){\n  var pos = p * N - i;\n  if (pos < 0) return; // 등장 전\n  c.style.transform = "translateY(" + (-pos * 12) + "px) scale("\n    + Math.max(0.92, 1 - pos * 0.04) + ")";\n  c.style.zIndex = N - i;\n});',
    explain: '활성 카드 + 이전 카드 동시 보임. 이전 카드는 -12px씩 위로 + scale 0.96·0.92로 축소되며 뒤에 쌓임. zIndex 역순으로 카드 덱 깊이감. 그라디언트 배경도 카드와 함께 축소.',
    kv: [
      { label: '의존성', value: 'Vanilla JS' },
    ...
    guide: '이전 카드가 뒤에 쌓여 진행 위치 시각화. 카드 3~5개 적정 — 6개 이상은 시각 혼잡.',
    recommendations: [
  ...
  // 09 — dissolve
  {
    ...
    snippetJS: 'var idx = Math.min(Math.floor(p * N), N - 1);\ncards.forEach(function(c, i){\n  c.classList.toggle("is-on", i === idx);\n});',
    explain: 'opacity + filter contrast/saturate 동시 변화. SVG fractalNoise overlay로 그레인 텍스처. 카드 전체(텍스트+이미지+그라디언트)가 800ms로 천천히 디졸브.',
    kv: [
    ...
    guide: '시네마틱 디졸브. SVG noise opacity 0.05~0.1. 그라디언트 배경도 함께 디졸브되어 색상 전환이 필름 느낌.',
    recommendations: [
  ...
  // 10 — carousel
  {
    ...
    summary: '활성 카드 가운데 + 인접 카드 ±80% scale 0.85 미니어처. 카드 흐름을 한눈에.',
    demo: {
      css: STAGE_CSS + '.scu-card { transform: translateX(100%) scale(0.85); opacity: 0.4; }',
    ...
    explain: '활성 카드(텍스트+이미지+그라디언트) 가운데 scale 1. 좌우 인접 카드 ±80% + scale 0.85 + opacity 0.35. 카드 전체가 가로 캐러셀로 배치.',
    kv: [
      { label: '의존성', value: 'Vanilla JS' },
      { label: '트리거', value: '스크롤 진행률 → activeIndex' },
    ...
    guide: '카드 시리즈를 한 번에 파악 가능. 인접 카드의 그라디언트 배경이 다르게 보여 색상 차이로 진행 위치 인지. 카드 3~5개 적정.',
    recommendations: [
...
function buildDemoHTML(p) {
  var isSticky = !!p.demo.sticky;
  var extraInner = p.demo.extraInnerHTML || '';
  var cardsHTML = isSticky ? stickyCardsMarkup() : absoluteCardsMarkup();
  var bodyContent = isSticky
...
function buildOverview() {
  ...
  return {
    title: '00. 카테고리 개요',
    blocks: [
      { type: 'heading', value: 'Scroll-Card-Update — base44.com 시각 매칭 v4 (Playwright 실측)' },
      ...
      { type: 'heading', value: '공통 디자인 토큰 (base44 Playwright 실측)' },
      {
        type: 'kv', columns: 2, items: [
          ...
          { label: '패턴 01 모델', value: '개별 sticky 카드 (CSS만, JS transform 없음)' },
          { label: '패턴 02~10', value: 'sticky-stage + absolute 카드 + JS applyReveal' },
      ...
      { type: 'heading', value: '읽기 가이드' },
      {
        type: 'structure', items: [
          ...
          { label: '작동 원리', tag: 'HOW', desc: '스크롤 진행률 → 카드 transform/opacity/clip 매핑' },
          { label: '정량 메타', tag: 'KV', desc: '의존성 / 트리거 / 매핑 / 카드 구조 / 시그니처' },
      ...
      {
        type: 'note',
        value: 'base44.com Playwright 실측 기반. 이너 카드 border-radius 9px, 그리드 59:41, 본문 30px/400, 번호 19px/400, 버튼 16px/500, 카드별 고유 그라디언트 4종 — 모두 정량 데이터에서 추출한 디자인 토큰.'
      }
    ]
  };
}
