  ...
  return (
    <section ref={sectionRef} className="global">
      ...
      <style jsx>{`
        ...
        .global__earth { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) scale(0.05); width: 720px; height: 720px; border-radius: 50%; background: radial-gradient(circle at 35% 35%, #5a8cd8 0%, #2868c8 25%, #1a4a90 50%, #0a2a60 75%, #000020 100%); box-shadow: inset -60px -60px 140px rgba(0,0,0,0.7), 0 0 100px rgba(80,140,220,0.35); overflow: hidden; z-index: 2; will-change: transform; }
        .global__earth::before { content: ''; position: absolute; inset: 0; border-radius: 50%; background: radial-gradient(ellipse 28% 18% at 30% 30%, rgba(255,255,255,0.55), transparent 60%), radial-gradient(ellipse 22% 14% at 70% 50%, rgba(220,240,255,0.45), transparent 60%), radial-gradient(ellipse 18% 10% at 50% 75%, rgba(200,220,255,0.4), transparent 60%); animation: earth-clouds 100s linear infinite; }
        .global__earth::after { content: ''; position: absolute; inset: 0; border-radius: 50%; background: radial-gradient(ellipse 18% 22% at 55% 40%, rgba(50,130,70,0.55), transparent 60%), radial-gradient(ellipse 12% 16% at 30% 60%, rgba(140,100,55,0.5), transparent 60%); }
        @keyframes earth-clouds { from { transform: rotate(0); } to { transform: rotate(360deg); } }
